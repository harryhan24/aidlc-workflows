...
같은 phase 안에서 여러 번 실행되는 skill은 `--scope <scope-name>`을 사용합니다(예: repo마다 실행되는 reverse-engineering). unscoped inception과의 차이점은 다음과 같습니다.

- workflow 라인에 `--scope <scope-name>`이 포함됩니다.
- artifact는 `inception/<skill>/` 대신 `inception/<skill>/<scope>/` 아래에 위치합니다.
- state key는 `<skill>:<scope>` 입니다(예: `reverse-engineering:payments-api`).

§3의 흐름은 동일하며, 한 번에 하나의 instance로 scoped된다는 점만 다릅니다.
